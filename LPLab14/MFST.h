...
#define MFST_TRACE7			out<<std::setw(4)<<std::left << state.lenta_position << ": "\
...
namespace MFST
{
	...
	struct Mfst							//магазинный автомат
	{
		...
		bool savestate(std::ofstream& out);					//сохранить состояние автомата
		bool resetstate(std::ofstream& out);					//восстановить состояние автомата
		bool push_chain(GRB::Rule::Chain chain);		//поместить цепочку правила в стек
		RC_STEP step(std::ofstream& out);						//выполнить шаг автомата
		bool start(Parm::PARM parm);						//запустить автомат
		bool savediagnosis(RC_STEP pprc_step);			//код завершения шага
		void printrules(std::ofstream& out);					//вывести послдеовательность правил
