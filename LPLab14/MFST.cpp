...
#pragma endregion
	Mfst::RC_STEP Mfst::step(std::ofstream& out)
	{
		...
		if (lenta_position < lenta_size)
		{
				...
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)
				{
					...
					else
					{
						MFST_TRACE4("TNS_NORULECHAIN/NS_NORULE")
							savediagnosis(NS_NORULECHAIN); rc = resetstate(out) ? NS_NORULECHAIN : NS_NORULE;
					};
				}
				else rc = NS_ERROR;
			}
			...
			else { MFST_TRACE4(TS_NOK / NS_NORULECHAIN) rc = resetstate(out) ? TS_NOK : NS_NORULECHAIN; };
		}
		else
		{
			rc = LENTA_END;
	...
	bool Mfst::resetstate(std::ofstream& out)
	{
		bool rc = false;
		MfstState state;
	...
	{
		bool rc = false;
		std::ofstream out(parm.syntax_file);
		MFST_TRACE_START
		...
		case LENTA_END:
		{
			...
			out << std::setw(4) << std::left << 0 << "всего строк " << lex.table[lex.size - 1].sn << ", синтаксический анализ выполнен без ошибок" << std::endl;
			rc = true;
			printrules(out);
			break;
		}
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			{
			case 170:
				throw Error::exception("Неверная структура программы", errid, lex.table[lpos].sn);
				break;
			case 171:
				throw Error::exception("Ошибочный оператор", errid, lex.table[lpos].sn);
				break;
			case 172:
				throw Error::exception("Ошибка в выражении ", errid, lex.table[lpos].sn);
				break;
			case 173:
				throw Error::exception("Ошибка в параметрах функции", errid, lex.table[lpos].sn);
				break;
			case 174:
				throw Error::exception("Ошибка в параметрах вызываемой функции", errid, lex.table[lpos].sn);
				break;
			case 175:
				throw Error::exception("Ошибочное выражение", errid, lex.table[lpos].sn);
				break;
			case 176:
				throw Error::exception("Ошибка в логическом выражении", errid, lex.table[lpos].sn);
				break;
			case 177:
				throw Error::exception("Ошибка в условном операторе", errid, lex.table[lpos].sn);
				break;
			}
			//sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d,	%s", e.get_id(), lex.table[lpos].sn , e.get_message());
			rc = buf;
		}
		return rc;
	}
